...
%% create S and R covariance matrices based on time windows

% pre- and post-"stimulus"
tidx = dsearchn(EEG.times',[ -1 0 1.5 ]');
...
for triali=1:EEG.trials
    ...
    R   = R + tmp*tmp'/diff(tidx(1:2));
    
    %%% R matrix
    tmp = EEG.data(:,tidx(2):tidx(3),triali);
    tmp = bsxfun(@minus,tmp,mean(tmp,2));
    S   = S + tmp*tmp'/diff(tidx(2:3));
end
